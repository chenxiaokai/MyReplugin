...
public class PMF {
    ...
    public static final void init(Application application) {
        ...
        //创建插件管理者
        //内部创建了 PmLocalImpl  PmInternalImpl等接口实现
        //各种坑名集合
        //PluginProcessPer 进程间通信 包含了:
             // PluginServiceService 【负责Server端的服务调度，提供等工作，是服务的提供方，核心类之一】
             // PluginContainers 【插件容器管理】，根据LaunchModeStates初始化各种坑
        sPluginMgr = new PmBase(application);

        //填充插件mPlugins集合，加载默认插件等
        sPluginMgr.init();

        //getLocal 对应 PmLocalImpl包含: query,load,startActivity,getActivityInfo等操作
        Factory.sPluginManager = PMF.getLocal();
        //getInternal 对应PmInternalImpl：内部框架使用，内部实际实现startActivity等
        Factory2.sPLProxy = PMF.getInternal();

        //Hook住ClassLoader 这里是系统唯一Hook点
