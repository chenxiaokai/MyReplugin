...
public class RePluginClassLoader extends PathClassLoader {
    ...
    public RePluginClassLoader(ClassLoader parent, ClassLoader orig) {
        ...
        //将orig中字段 拷贝到  RepluginClassLoader对象上,让系统以为用RepluginClassLoader还是orig原来的ClassLoader
    ...
    private void copyFromOriginal(ClassLoader orig) {
        ...
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.GINGERBREAD_MR1) {
            ...
            // 以下方法在较慢的手机上用时：8ms左右
            //从orig中取出值 赋值 给 RePluginClassLoader
    ...
    protected Class<?> loadClass(String className, boolean resolve) throws ClassNotFoundException {
        //
        Class<?> c = null;
        //这里就是Hook点，先使用插件的PluginDexClassLoader加载
        c = PMF.loadClass(className, resolve);
        if (c != null) {
            return c;
        }
        //只有在插件没有找到相应的类，才使用系统原来的PatchClassLoader加载宿主中的类
        try {
