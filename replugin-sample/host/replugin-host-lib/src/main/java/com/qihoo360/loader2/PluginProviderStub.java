...
public class PluginProviderStub {
    ...
    private static final IBinder proxyFetchHostBinder(Context context, String selection) {
        ...
        Cursor cursor = null;
        try {
            //ProcessPitProviderPersist 运行在 :GuardService 进程中
            //uri 经过拼接后： content://com.qihoo360.replugin.sample.host.loader.p.main/main
            /*
             这个就是编译后 manifest中的 provider 注册
                    <provider
             ...
             */
            //查询的Provider 就是 ProcessPitProviderPersist  这个 Provider 在编译的时候已经 注册到 Manifest中去了。
    ...
    static final boolean proxyStartPluginProcess(Context context, int index) {
        //尝试去访问一个ContentProvider，如果它没有运行，则在新的进程中启动它，而这个进程刚好就是上一节中通过PackageInifo.mMetaData匹配出来的进程（Replugin默认提供的三个自定义进程之一）。
