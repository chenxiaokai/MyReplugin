    ...
    public void regReceiver(String plugin, Map rcvFilMap) throws RemoteException {
        ...
        for (HashMap.Entry<String, List<IntentFilter>> entry : receiverFilterMap.entrySet()) {
            if (mReceiverProxy == null) {
                //第一步: 创建PluginReceiverProxy对象，给它添加一个保存Action的Map对象mActionPluginComponents
            ...
            // 第二步: 遍历插件中的广播，并将广播以及广播的IntentFilter信息都保存到mActionPluginComponents中
            if (filters != null) {
                for (IntentFilter filter : filters) {
                    ...
                    // 注册 Receiver
                    //第三步: PluginReceiverProxy实际上就是一个广播，所以将它注册到Android系统中，
                    // 并将插件中所有广播的所有IntentFilter都添加到这个广播中。因此当系统发送广播时，
                    // 所有匹配这些IntentFilter的广播都会首先被PluginReceiverProxy接收到
