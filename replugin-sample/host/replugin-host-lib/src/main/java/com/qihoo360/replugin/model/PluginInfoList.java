...
public class PluginInfoList implements Iterable<PluginInfo> {
    ...
    public boolean load(Context context) {
         try {
            // 1. 新建或打开文件 Constant.LOCAL_PLUGIN_APK_SUB_DIR就是插件安装以后存放目录
             /**
              * String cachePath1 = getExternalCacheDir().getPath();  /storage/emulated/0/Android/data/<packagename>/cache
              * String cachePath2 = getCacheDir().getPath();  /data/user/0/<packagename>/cache
              * File file = getDir("p_a", 0);   /data/user/0/<packagename>/app_p_a  (app_ 是系统加上的)
              */
