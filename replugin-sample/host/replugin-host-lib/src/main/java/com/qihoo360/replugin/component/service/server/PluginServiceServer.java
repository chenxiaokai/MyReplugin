...
//        ProcessRecord callerPr = retrieveProcessRecordLocked(client);
        ...
        //PluginServiceServer.startServiceLocked先通过retrieveServiceLocked获取或者创建一个ServiceRecord对象
        final ServiceRecord sr = retrieveServiceLocked(intent);
        if (sr == null) {
            return null;
        }
        //然后用installServiceIfNeededLocked开始运行Service的生命周期函数
    ...
    private boolean installServiceIfNeededLocked(final ServiceRecord sr) {
        ...
        try {
            Boolean result = ThreadUtils.syncToMainThread(new Callable<Boolean>() {
                ...
                public Boolean call() {
                    /*
                     * installServiceLocked函数，加载Service类，通过反射创建Service对象，并利用attachBaseContextLocked函数反射调用Service.attachBaseContext，
                     * 注意这里会将插件的全局PluginContext赋值给Service，然后还调用了Service.onCreate函数，最后一步将开启Service坑位，
                     * 启动Service坑位只是为了让进程被回收的可能性降低（请参考android系统资源回收的优先级）。
                     */
