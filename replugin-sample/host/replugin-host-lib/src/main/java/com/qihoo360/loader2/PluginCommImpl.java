...
public class PluginCommImpl {
     ...
     * @return
     */
    public ComponentName loadPluginActivity(Intent intent, String plugin, String activity, int process) { //寻找坑位Activity
        ...
        try {
            ...
            ai = getActivityInfo(plugin, activity, intent);  //如果是第一次去获取信息，会首先去加载插件的Dex文件以及资源等，并创建PluginDexClassLoader
            if (ai == null) {
                if (LOG) {
            ...
            if (LOG) {
                //theme: intent.putExtra(com.qihoo360.replugin.sample.demo1.MainActivity, 0);
            ...
            // 容器选择（启动目标进程）
            IPluginClient client = MP.startPluginProcess(plugin, process, info); //如果有必要的话，一般默认会使用UI进程）
            if (client == null) {
                return null;
            }

            // 常驻进程中分配坑位 只有分配到坑位，插件的Activity才可以启动，这是一个IPC过程
            container = client.allocActivityContainer(plugin, process, ai.name, intent);
            if (LOG) {
                // alloc success: container=com.qihoo360.replugin.sample.host.loader.a.ActivityN1NRNTS2 plugin=com.qihoo360.replugin.sample.demo1 activity=com.qihoo360.replugin.sample.demo1.MainActivity
                // alloc success: container=com.qihoo360.replugin.sample.host.loader.a.ActivityN1NRNTS4 plugin=com.qihoo360.replugin.sample.demo1 activity=com.qihoo360.replugin.sample.demo1.activity.standard.StandardActivity
                // alloc success: container=com.qihoo360.replugin.sample.host.loader.a.ActivityN1STPNTS1 plugin=com.qihoo360.replugin.sample.demo1 activity=com.qihoo360.replugin.sample.demo1.activity.single_top.SingleTopActivity1
