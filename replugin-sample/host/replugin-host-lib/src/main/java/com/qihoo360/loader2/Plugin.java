...
class Plugin {
    ...
    final boolean load(int load, boolean useCache) {
        ...
        // 尝试在此处调用插件的Application.onCreate方法
    ...
    private boolean loadLocked(int load, boolean useCache) {
        ...
        if (LOG) {
            // 打开插件 demo1
            // loadLocked(): Ready to lock! logtag = try1; pn = demo1
        ...
        //真正的加载
        //主要加载 插件Dex文件 资源 以及so文件等等
        boolean rc = doLoad(logTag, context, parent, manager, load);
        if (LOG) {
            // 打开插件 demo1
            // load /data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3/demo1-10-10-104.jar 255473236 c=3 rc=true delta=107
    ...
    private final boolean doLoad(String tag, Context context, ClassLoader parent, PluginCommImpl manager, int load) {
        if (mLoader == null) {
            ...
            if (mInfo.getType() == PluginInfo.TYPE_BUILTIN) { //内置插件
                //dir 插件存放目录 plugins_v3 插件存放目录  dir = /data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3
                File dir = context.getDir(Constant.LOCAL_PLUGIN_SUB_DIR, 0);
                // dexdir = /data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3_odex
                File dexdir = mInfo.getDexParentDir();
                //dstName = demo1-10-10-104.jar
                ...
                // file = /data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3/demo1-10-10-104.jar
            ...
            if (!mLoader.loadDex(parent, load)) { //dex 加载过程
                return false;
            }
            ...
            if (load == LOAD_APP) {
                ...
                //Dex文件加载完成以后，要运行Plugin还需要初始化Plugin的运行环境相关的类

                //loadEntryLocked函数，这个函数负责初始化Plugin的运行环境。
    ...
    private boolean loadEntryLocked(PluginCommImpl manager) {
        if (mDummyPlugin) {
            ...
            if (LOG) {
                //打开插件 demo1
                // Plugin.loadEntryLocked(): Load entry, info=PInfo { <demo1:104(4)> [P-N] [DEX_EXTRACTED] [USED] js={"pkgname":"com.qihoo360.replugin.sample.demo1","ali":"demo1","name":"demo1","low":10,"high":10,"ver":104,"verv":2814792716779624,"path":"\/data\/user\/0\/com.qihoo360.replugin.sample.host\/app_plugins_v3\/demo1-10-10-104.jar","type":1,"frm_ver":4} dex=/data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3_odex/demo1-10-10-104.dex nlib=/data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3_libs/demo1-10-10-104 }
