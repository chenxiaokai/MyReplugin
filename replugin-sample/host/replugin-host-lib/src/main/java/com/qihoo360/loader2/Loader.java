...
class Loader {
    ...
    //获取Dex中的组件的信息，包括Manifest中的组件属性，比如进程属性，TaskAffinity属性，注册静态广播等等
    final boolean loadDex(ClassLoader parent, int load) {
        try {
            ...
            if (mPackageInfo == null) {
                // PackageInfo 获取四大组件和Application 和应用程序的一些信息
                ...
                if (LOG) {
                    // 打开插件demo1
                    //get package archive info, pi=PackageInfo{a1447a8 com.qihoo360.replugin.sample.demo1}
                    LogDebug.d(PLUGIN_TAG, "get package archive info, pi=" + mPackageInfo);
                }
                // mPath = /data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3/demo1-10-10-104.jar
                ...
                // ld = /data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3_libs/demo1-10-10-104
...
//                }
            ...
            if (mClassLoader == null) {
                ...
                // load /data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3/demo1-10-10-104.jar ...
                ...
                //load /data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3/demo1-10-10-104.jar = com.qihoo360.replugin.PluginDexClassLoader[DexPathList[
                // [zip file "/data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3/demo1-10-10-104.jar"],
                // nativeLibraryDirectories=[/data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3_libs/demo1-10-10-104, /vendor/lib64, /system/lib64]]]
            ...
            // 其实每一个Plugin的Activity都会创建一个PluginContext对象，并使用相同的ClassLoader和Resources，因此在Plugin中就可以加载相关的类和使用资源了，跟原生程序一样。
...
//            }
    ...
    final boolean invoke2(PluginCommImpl x) {
        try {
            ...
            //这里有一个参数是ClassLoader，这个ClassLoader是Host中的RepluginClassLoader，有了它，Plugin才能找到Host当中的类并调用这些类的方法。
            ...
            if (LOG) {
                // Loader.invoke2(): plugin=/data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3/demo1-10-10-104.jar,
                // plugin.binder.cl=com.qihoo360.replugin.PluginDexClassLoader[DexPathList[[zip file "/data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3/demo1-10-10-104.jar"],nativeLibraryDirectories=[/data/user/0/com.qihoo360.replugin.sample.host/app_plugins_v3_libs/demo1-10-10-104, /vendor/lib64, /system/lib64]]]
    ...
    private HashMap<String, String> getConfigProcessMap(ApplicationInfo appInfo) {
         ...
         */
        HashMap<String, String> processMap = new HashMap<>();
        Bundle bdl = appInfo.metaData;  //插件manifest中 metaData元素
    ...
    private void adjustPluginProcess(ApplicationInfo appInfo) {
        ...
        //doAdjust函数用Replugin内置的进程名替换掉用户在插件中自定义的进程名并保存在ComponentList中的ComponentIinfo中
        ...
        if (LOG) {
            //--- 调整插件中组件的进程 END --- com.qihoo360.replugin.sample.host
