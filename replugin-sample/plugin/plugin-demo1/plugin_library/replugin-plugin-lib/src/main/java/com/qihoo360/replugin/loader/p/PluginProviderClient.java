...
public class PluginProviderClient {
    ...
    public static Uri insert(Context c, Uri uri, ContentValues values) {
        ...
        //如果RePluginFramework没有初始化，说明当前的插件是被当作普通应用在使用，所以直接使用原生的provider操作。
        if (!RePluginFramework.mHostInitialized) {
            return c.getContentResolver().insert(uri, values);  //原生操作
        }

        try {
            // 反射调用replugin-host-lib中的PluginProviderClient.insert
    ...
    public static class ProxyRePluginProviderClientVar {
        ...
        //这个ClassLoader是Host中的RepluginClassLoader
