...
public class LoaderActivityInjector extends BaseInjector {
    ...
    /* LoaderActivity 替换规则 替换成 replugin-plugin-lib中com.qihoo360.replugin.loader.a 的 Activity */
    ...
    def injectClass(ClassPool pool, String dir, Map config) {
        ...
        new ManifestAPI().getActivities(project, variantDir).each { //获取library中 所有 activity
            // 处理没有被忽略的 Activity
    ...
    private def handleActivity(ClassPool pool, String activity, String classesDir) {
        //classesDir 是更改后activity存放目录 E:\github\RePlugin-2.2.0\....\app\intermediates\classes\release\
        ...
        try {
            ...
            //从文件流中加载.class文件，创建一个CtClass实例，这个实例表示.class文件对应的类或接口
...
*/
            ...
            if (superCls != null) {
                ...
                ctCls.getDeclaredMethods().each { outerMethod ->
                    //调用CtMethod的instrument()，方法体会被逐行进行扫描，从第一行扫描到最后一行
