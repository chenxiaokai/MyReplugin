...
public class ReClassTransform extends Transform {
    ...
    @Override
    String getName() {
        //本质上是在merge{ProductFlavor}{BuildType}Assets Task 之后，transformClassesWithDexFor{ProductFlavor}{BuildType} Transform 之前,
    ...
    void transform(Context context,
        ...
        CommonData.appModule = config.appModule  // 默认 :app
        CommonData.ignoredActivities = config.ignoredActivities  //执行 LoaderActivity 替换时，用户声明不需要替换的 Activity
    ...
    def doTransform(Collection<TransformInput> inputs,
        ...
        /* 初始化 ClassPool */
        //initClassPool(...)方法主要的工作：添加编译时引用到的类到 ClassPool，同时记录要修改的 jar 到 includeJars。方便后续拿到这些class文件去修改
    ...
    def initClassPool(Collection<TransformInput> inputs) {
        Util.newSection()
        def pool = new ClassPool(true) //true ClassPool 将搜索 系统 path
        ...
        Util.getClassPaths(project, globalScope, inputs, includeJars, map).each {
            ...
            println "    $it"
            pool.insertClassPath(it) //增加到搜寻目录
        }
        pool
    }
